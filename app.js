...
Days turned into weeks as the adventure continued. New friends were made, challenges were overcome, and the protagonist grew wiser with each passing moment.`;
    ...
    async readNextPhrase() {
        ...
        try {
            console.log('🎯 Starting readNextPhrase for:', phrase.substring(0, 50) + '...');
            const audioUrl = await this.generateSpeech(phrase);
            
            if (!audioUrl) {
                console.error('❌ No audio URL returned from generateSpeech');
            ...
            this.currentAudio.addEventListener('error', (e) => {
                ...
                this.progressText.textContent = `❌ ${errorMsg}`;
                if (this.isIOS()) {
    ...
    async generateSpeech(text) {
        ...
        console.log(`🎤 TTS Provider selection:`, {
            isIOS: this.isIOS(),
            preferKokoro,
        ...
        try {
            if (useKokoro) {
                console.log('📞 Calling Kokoro TTS...');
    ...
    async generateGoogleSpeech(text) {
        ...
        console.log('✅ Google TTS URL created:', audioUrl.substring(0, 100) + '...');
        return audioUrl;
    }

    async generateKokoroSpeech(text) {
        console.log('🔊 Generating Kokoro TTS for:', text.substring(0, 50) + '...');
        console.log('📤 Kokoro request:', { voice: this.selectedVoice, speed: this.speed });
